...
<div class="p-4 sm:p-6" x-data="documentPage()"> 
								...
								<tr class="hover:bg-gray-50">
									...
									<td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
										<div class="flex justify-center items-center space-x-3">
											...
											<button @click="fileModalOpen = true; documentFiles = {{ json_encode($document->files) }}" class="text-gray-500 hover:text-blue-700" title="Lihat File">
											...
											{{-- TOMBOL REVIEW (DIPERBAIKI) --}}
											@can('review', $document)
												<button @click="reviewModalOpen = true; actionUrl = '{{ route('documents.storeReview', ['package' => $package->id, 'shop_drawing' => $document->id]) }}'; documentTitle = '{{ addslashes($document->title) }}'" class="text-blue-600 hover:text-blue-800" title="Review Dokumen">
	...
	{{-- =================================== --}}
    {{-- AWAL DARI MODAL UNTUK MELIHAT FILE --}}
    {{-- =================================== --}}
    <div x-show="fileModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div @click.away="fileModalOpen = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto">
                {{-- Alpine.js akan menampilkan daftar file di sini --}}
    ...
    {{-- =================================== --}}
    {{-- AKHIR DARI MODAL UNTUK MELIHAT FILE --}}
    {{-- =================================== --}}
	

    {{-- ====================================================== --}}
    {{-- AWAL DARI MODAL REVIEW BARU --}}
    {{-- ====================================================== --}}
    <div x-show="reviewModal.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
        <div @click.away="closeReviewModal()" class="bg-gray-50 rounded-lg shadow-xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col">
            <div class="bg-white p-4 border-b rounded-t-lg">
                <h2 class="text-xl font-bold">Formulir Review Shop Drawing</h2>
                <p class="text-sm text-gray-500">Dokumen: <strong x-text="reviewModal.documentTitle"></strong></p>
            </div>

            <div class="p-6 overflow-y-auto space-y-6">
                {{-- Tampilan Loading --}}
                <div x-show="reviewModal.loading" class="text-center py-10">
                    <p class="text-gray-500">Memuat detail dokumen...</p>
                </div>
                ...
                <div x-show="!reviewModal.loading" class="space-y-6">
                    {{-- 1. Daftar Gambar --}}
                    <div>
                        <h3 class="font-semibold text-gray-800 border-b pb-2 mb-2">Daftar Gambar</h3>
                        <div class="space-y-1 text-sm">
                            <ul class="list-disc pl-5">
                                <template x-for="drawing in reviewModal.details.drawings" :key="drawing.id">
                                    <li><span x-text="drawing.drawing_number" class="font-mono bg-gray-200 px-1 rounded"></span> - <span x-text="drawing.drawing_title"></span></li>
                                </template>
                            </ul>
                        </div>
                    </div>

                    {{-- 2. Pekerjaan Terkait & Status Kelengkapan --}}
                    <div>
                        <h3 class="font-semibold text-gray-800 border-b pb-2 mb-2">Pekerjaan Terkait & Status Kelengkapan</h3>
                             ...
                             <template x-for="item in reviewModal.details.rab_items" :key="item.id">
                                <div class="flex justify-between items-center p-2 bg-white rounded-md border">
                                    ...
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full"
                                        :class="item.pivot.completion_status === 'lengkap' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'"
                                        x-text="item.pivot.completion_status === 'lengkap' ? 'Lengkap' : 'Belum Lengkap'">
                                    </span>
                                </div>
                            </template>
                        </div>
                    ...
                    {{-- 3. Form Keputusan & Disposisi --}}
                    <form :action="reviewModal.actionUrl" method="POST">
                        @csrf
                        <div class="space-y-4 bg-white p-4 rounded-md border">
                            <h3 class="font-semibold text-gray-800">Keputusan Review</h3>
                            <div>
                                <label for="status" class="block text-sm font-medium text-gray-700">Status Review</label>
                            ...
                            <div>
                                <label for="disposition" class="block text-sm font-medium text-gray-700">Disposisi</label>
                                <select id="disposition" name="disposition" class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md" required>
                                    ...
                                    {{-- Opsi ini akan kita buat dinamis setelah mendapat info struktur jabatan --}}
                                    <option value="team_leader">Lanjutkan ke Team Leader</option>
                                    <option value="owner">Lanjutkan ke Owner</option>
                                </select>
                            </div>
                        </div>
                        <div class="mt-6 flex justify-end space-x-3 bg-white p-4 -mx-6 -mb-6 rounded-b-lg border-t">
