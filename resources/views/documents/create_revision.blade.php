...
<div class="p-4 sm:p-6" x-data="revisionForm()">
    <header class="bg-white shadow p-4 rounded-lg mb-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-2">Revisi untuk Dokumen: <span class="font-semibold">{{ $shop_drawing->document_number }} - {{ $shop_drawing->title }}</span></p>
            </div>
    ...
    <main class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('documents.store_revision', ['package' => $package->id, 'shop_drawing' => $shop_drawing->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6 border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="hidden"> {{-- Category disembunyikan karena selalu shop_drawing --}}
            ...
            <div class="mb-6 border-b pb-4">
                ...
                <p class="mt-2 text-xs text-gray-500">Unggah satu atau lebih file PDF untuk revisi ini. Maks. 10MB per file.</p>
            ...
            <div class="mb-6 border-b pb-4">
                ...
                <template x-for="(drawing, index) in drawings" :key="index">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 items-end bg-gray-50 p-4 rounded-md">
                        <div>
                            ...
                            <input type="text" :id="`drawing_number_${index}`" :name="`drawings[${index}][number]`"
                                class="mt-1 block w-full border rounded-md shadow-sm py-2 px-3"
                                x-model="drawing.number" required>
                        </div>
                        <div class="md:col-span-2">
                            <label :for="`drawing_title_${index}`" class="block text-sm font-medium text-gray-700">Judul Gambar</label>
                            <div class="flex">
                                <input type="text" :id="`drawing_title_${index}`" :name="`drawings[${index}][title]`"
                                    class="mt-1 block w-full border rounded-md shadow-sm py-2 px-3"
                                    x-model="drawing.title" required>
                ...
                <button type="button" @click="addDrawing()" class="mt-4 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-md text-sm">
                    + Tambah Gambar
                </button>
            </div>
            ...
            <div class="mb-6 border-b pb-4">
                ...
                <template x-for="(rabItem, index) in rabItems" :key="index">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 items-center bg-gray-50 p-4 rounded-md">
                        <div class="md:col-span-2">
                            <label :for="`rab_item_name_${index}`" class="block text-sm font-medium text-gray-700">Nama Pekerjaan</label>
                            <input type="text" :id="`rab_item_name_${index}`" :name="`rab_items[${rabItem.id}][name]`"
                                class="mt-1 block w-full border rounded-md shadow-sm py-2 px-3 bg-gray-100"
                                x-model="rabItem.name" readonly>
                            <input type="hidden" :name="`rab_items[${rabItem.id}][id]`" x-model="rabItem.id">
                        </div>
                        <div>
                            <label :for="`rab_item_status_${index}`" class="block text-sm font-medium text-gray-700">Status Kelengkapan</label>
                            <select :id="`rab_item_status_${index}`" :name="`rab_items[${rabItem.id}][completion_status]`"
                                class="mt-1 block w-full border rounded-md shadow-sm py-2 px-3"
                                x-model="rabItem.pivot.completion_status">
                ...
                <div x-show="rabItems.length === 0" class="text-gray-500 text-sm">Tidak ada item RAB yang terkait dengan pengajuan sebelumnya.</div>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('revisionForm', () => ({
            drawings: @json($shop_drawing->drawingDetails->map(fn($dd) => ['number' => $dd->drawing_number, 'title' => $dd->drawing_title]))->isNotEmpty() ? @json($shop_drawing->drawingDetails->map(fn($dd) => ['number' => $dd->drawing_number, 'title' => $dd->drawing_title])) : [{ number: '', title: '' }],
