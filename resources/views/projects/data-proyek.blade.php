    ...
    <div class="space-y-6">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="space-y-6">
                ...
                {{-- LOOPING UNTUK SEMUA PERUSAHAAN --}}
                @forelse($project->companies as $company)
                    @can('viewCompanyDetails', [$project, $company])
                        @php
                            $isOwner = $company->type === 'owner';
                            $isContractor = $company->type === 'contractor';
                            
                            $title = $company->pivot->role_in_project; // Default title
                            if ($isOwner) $title = 'Pemilik (Owner)';
                            if ($isContractor) $title = $company->pivot->role_in_project; // e.g. "Kontraktor Paket A"
                            // Anda bisa tambahkan logika lain untuk MK, Perencana, dll jika perlu
                        @endphp

                        <div class="border-l-4 {{ $isOwner ? 'border-blue-500' : 'border-gray-400' }} pl-4 py-2">
                            <div class="flex justify-between items-start mb-2">
                                {{-- GANTI: Baris judul perusahaan (h3) --}}
								<h3 class="font-bold text-md text-gray-800 flex items-center space-x-3">
									<img src="{{ $company->logo_url }}" alt="Logo {{ $company->name }}" class="h-8 w-8 object-contain rounded-sm">
                    ...
                    @endcan
                @empty
                    <p class="text-sm text-gray-500">Belum ada instansi yang ditambahkan ke proyek ini.</p>
                @endforelse
            </div>
        </div>
    </div>
