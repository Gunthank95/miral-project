...
<div class="p-4 sm:p-6">
    ...
    <div class="space-y-6">
         <class="space-y-6">
        {{-- SEKSI 1: INFORMASI UMUM (OTOMATIS) --}}
        ...
        {{-- SEKSI 2: LAPORAN CUACA --}}
        {{-- Form untuk menambah data cuaca baru --}}
			<div class="bg-white rounded shadow p-4">
            <h2 class="text-xl font-semibold mb-4 border-b pb-2">2. Laporan Cuaca</h2>
            <div id="weather-success-message" class="hidden bg-green-100 text-green-700 p-3 rounded mb-4"></div>
            ...
            <form action="{{ route('daily_reports.weather.store', $report->id) }}" method="POST" class="border-t pt-4">
				@csrf
				<div class="flex flex-wrap items-end gap-4">
					{{-- Grup Jam --}}
					<div class="flex-grow sm:flex-grow-0 sm:w-1/4">
						...
						<div class="flex items-center space-x-2 mt-1">
							...
							<button type="button" id="time-now-btn" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 flex-shrink-0">Now</button>
						</div>
					</div>
					{{-- Grup Kondisi Cuaca --}}
					<div class="flex-grow sm:flex-grow-0 sm:w-1/4">
        ...
        {{-- SEKSI 3: LAPORAN TENAGA KERJA (DENGAN TAMPILAN BARU) --}}
        <div class="bg-white rounded shadow p-4">
            <h2 class="text-xl font-semibold mb-4 border-b pb-2">3. Laporan Tenaga Kerja</h2>
            <div id="personnel-success-message" class="hidden bg-green-100 text-green-700 p-3 rounded mb-4"></div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Kolom Kiri: KONTRAKTOR --}}
                <div>
                    ...
                    <form class="personnel-form border-t pt-2 flex items-end space-x-2" data-company-type="Kontraktor">
                        @csrf
                        <div class="flex-grow"><input type="text" name="role" required class="w-full border rounded px-2 py-1 text-sm" placeholder="Jabatan"></div>
                        <div class="w-1/4"><input type="number" name="count" required class="w-full border rounded px-2 py-1 text-sm" placeholder="Jumlah"></div>
                        <div><button type="submit" class="bg-blue-500 text-white px-3 py-1 rounded text-sm">+</button></div>
                    </form>
                </div>

                {{-- Kolom Kanan: MK / PENGAWAS --}}
                <div>
        ...
        {{-- SEKSI 4: AKTIVITAS PEKERJAAN (SEKARANG FUNGSIONAL) --}}
        <div class="bg-white rounded shadow p-4">
            ...
            {{-- Tabel untuk menampilkan aktivitas yang sudah ditambahkan --}}
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="text-left px-4 py-2">Uraian Pekerjaan</th>
                            <th class="text-center px-4 py-2">Progres Volume</th>
                            <th class="text-center px-4 py-2">Tenaga Kerja</th>
                        ...
                        @forelse ($report->activities as $activity)
                            <tr class="border-t">
                                ...
                                <td class="px-4 py-2 text-right">
                                    {{-- Tombol edit/hapus bisa kita tambahkan di sini nanti --}}
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center p-4 text-gray-500">
                                    Belum ada aktivitas pekerjaan yang ditambahkan untuk laporan ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
